...
注：本例是示例 Demo ,仅在 首页 --> PLRTCStreamingKitDemo --> 主播 加入了 FaceUnity 效果，如需更多接入用户可参考本例自行实现。
...
在 `PLRTCStreamingChiefViewController.m` 的 `viewDidLoad:` 方法中调用快速加载道具函数，该函数会创建一个美颜道具及指定的贴纸道具。
...
在 `PLRTCStreamingChiefViewController.m` 的 `mediaStreamingSession: cameraSourceDidGetPixelBuffer:` 方法中进行图像处理：
